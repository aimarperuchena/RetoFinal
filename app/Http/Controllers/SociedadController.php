...
class SociedadController extends Controller
{
  ...
  public function reservaFecha(Request $request, $sociedad_id)
  {
    ...
    $sociedad = Sociedad::find($sociedad_id);
    $mesas = DB::select('select * from mesa where sociedad_id="$sociedad->id" and id not in(select mesa_id from mesa_reserva where reserva_id in(select id from reserva where fecha="$newDate" and sociedad_id="$sociedad_id" and tipo_id = "$request->tipo"))');
